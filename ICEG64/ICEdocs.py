...
utf8_characters = [chr(code_point) for code_point in range(0x0, 0x10FFFF + 1)]
...
def create_saltBin(salt):
    ...
    while len(var) < int(salt):
        var = (var + str(randomNum()))
    var = bin(int(var))[2:int(salt)]

    return var

def create_saltInt(salt):
    #just create some random characters (for only encoding, generated always)
    var=(rr.randint(0, 64))
    var= int((var/8.23)*(randomNum()/8.79))
    var = str(var)+str(randomNum())
    while len(var) < int(salt):
        var = (var + str(randomNum()))
    var = str(var)[:int(salt)]

    return var

def create_saltStr(salt):
    #generate random characters
    varB = create_saltBin(salt*5)
    varS = convertSfB(varB, 0)
    while len(varS) < salt:
        varB = varB + create_saltBin(salt*5)
        varS = convertSfB(varB, 0)
    return varS[:salt]

def convertStB(input, salt):
    var = input
    #encode input (str) to binary
    var = ''.join(format(ord(char), '08b') for char in str(input))
    var = create_saltBin(int(salt)) + var
    return var

def convertSfB(input, salt):
    var = input
    var = var[int(salt):]
...
def convertStI(input, saltInput):
    var = input
    #encode input (str) to integer
    var = ''.join(format(ord(char), '08b') for char in str(input))
    var = int(var, 2)
    var = create_saltInt(int(saltInput)) + str(var)

    return var
    
def convertSfI(input, salt):
    var = input
    var = var[int(salt):]
...
def ICEG64(input, saltInput):
    ...
    return (salt[:saltInput] + var)

def deICEG64(input, saltInput):
    var = input[saltInput:]
...
def ICEG64Fib(input, salt):
    var = ICEG64(input, 0)
    fibvar = generate_fibonacci(len(var))
    for i in fibvar:
        var = var[:i] + create_saltStr(salt) + var[i:]

    return create_saltStr(salt)+var

def deICEG64Fib(input, salt):
    var = input[salt:]
...
def ICEG32(input, salt):
    var = translate(input, translation_dict)
    var = convertStI(var, 0)

    salt = create_saltStr(salt)
    salt = convertStI(salt, 0)

    return salt[:salt] + var

def deICEG32(input, salt):
    var = input[salt:]
    var = convertSfI(var, 0)
    var = translate_back(var, translation_dict)
...
def help():
    ...
    print("  • webhelp - open help page in browser")
    ...
    print("  saltB\t\t\t\t\tcreate_saltBin()\t\tGenerate salt length random binary")
    print("  saltI\t\t\t\t\tcreate_saltInt()\t\tGenerate salt length random integer")
    print("  saltS\t\t\t\t\tcreate_saltStr()\t\tGenerate salt length random string")
    print("  e\t\t\t\t\tconvertStB()\t\t\tconvert string to binary, insert salt length salt at the beginning")
    print("  d\t\t\t\t\tconvertSfB()\t\t\tconvert string from binary, cut salt length salt from the beginning")
    print("  eI\t\t\t\t\tconvertStI()\t\t\tconvert string to integer, insert salt length salt at the beginning")
    print("  dI\t\t\t\t\tconvertSfI()\t\t\tconvert string from integer, cut salt length salt from the beginning")
    print("  _NONE_\t\t\t\trandomNum()\t\t\tgenerate a pseudo-random number, based on time")
    print("  encode\t\t\t\tencode()\t\t\tConvert string to integer, multiply it by 29, and translate it, than insert salt length salt")
    print("  decode\t\t\t\tdecode()\t\t\tCut salt lenght salt off, divide by 29 convert to string")
    ...
    print("  ICE\t\t\t\t\tICEG64()\t\t\tTranslate string, convert it to integer, translate it again, insert salt lenght salt")
    print("  deICE\t\t\t\t\tdeICEG64()\t\t\tRemove salt, transalte back, convert integer to string, translate it again")
    print("  ICEfib\t\t\t\tICEG64Fib()\t\t\t[NOT FINISHED YET!] translate it, convert it to integer, translate it, insert salt length salt every fibonacci position")
    print("  deICEfib\t\t\t\tdeICEG64Fib()\t\t\t[NOT FINISHED YET!] Remove salt lenght salt from every fibonacci position, transalate it back, convert string form integer, translate it back")
    print("  _NONE_\t\t\t\tgenerate_fibonacci()\t\tGenerate fibonacci numbers that are below X. Created for ICEG64fib")
    print("  ICE32\t\t\t\t\tICEG32()\t\t\tLighter version of ICEG64. Translate string, than convert it to integer. Insert salt length salt to the beginning.")
